...
#   ./mcp/searcher/install.sh /Users/dylanjensen/Desktop/Neural_Helix
...
MCP_SCRIPT="$ROOT/mcp/searcher/server.py"
MCP_CWD="$ROOT/mcp"

echo "ROOT=$ROOT"
echo "Installing Searcher MCP Server..."

# 1. Move searcher.md to agents/main/skills
mkdir -p "$SKILLS_DIR"
if [ -f "$SCRIPT_DIR/searcher.md" ]; then
  mv "$SCRIPT_DIR/searcher.md" "$SKILLS_DIR/"
  echo "Moved searcher.md to $SKILLS_DIR"
else
  echo "Warning: $SCRIPT_DIR/searcher.md not found (may already be installed)"
fi
...
# 3. Append searcher config to mcporter.json
JQ=""
...
if [ ! -f "$MCPPER_JSON" ]; then
  echo "Creating $MCPPER_JSON with searcher config"
...
' "$MCPPER_JSON" "$MCPPER_JSON.new" > "$MCPPER_JSON.tmp" && mv "$MCPPER_JSON.tmp" "$MCPPER_JSON"
  ...
  echo "Appended searcher to $MCPPER_JSON"
fi
