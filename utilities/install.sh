...
#   ./mcp/utilities/install.sh /Users/dylanjensen/Desktop/Neural_Helix
...
MCP_SCRIPT="$ROOT/mcp/utilities/utilities.py"
MCP_CWD="$ROOT/mcp"
...
if [ -f "$SCRIPT_DIR/utilities.md" ]; then
  ...
  echo "Moved utilities.md to $SKILLS_DIR"
else
  echo "Warning: $SCRIPT_DIR/utilities.md not found (may already be installed)"
fi
...
# 3. Append utilities config to mcporter.json
JQ=""
...
if [ ! -f "$MCPPER_JSON" ]; then
  echo "Creating $MCPPER_JSON with utilities config"
...
' "$MCPPER_JSON" "$MCPPER_JSON.new" > "$MCPPER_JSON.tmp" && mv "$MCPPER_JSON.tmp" "$MCPPER_JSON"
  ...
  echo "Appended utilities to $MCPPER_JSON"
fi
