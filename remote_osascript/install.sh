...
#   ./mcp/remote_osascript/install.sh /Users/dylanjensen/Desktop/Neural_Helix
...
MCP_SCRIPT="$ROOT/mcp/remote_osascript/remote_osascript.py"
MCP_CWD="$ROOT/mcp"

echo "ROOT=$ROOT"
echo "Installing Remote osascript MCP Server..."

# 1. Move remote_osascript.md to agents/main/skills
mkdir -p "$SKILLS_DIR"
if [ -f "$SCRIPT_DIR/remote_osascript.md" ]; then
  mv "$SCRIPT_DIR/remote_osascript.md" "$SKILLS_DIR/"
  echo "Moved remote_osascript.md to $SKILLS_DIR"
else
  echo "Warning: $SCRIPT_DIR/remote_osascript.md not found (may already be installed)"
fi
...
# 3. Append remote_osascript config to mcporter.json
JQ=""
...
if [ ! -f "$MCPPER_JSON" ]; then
  echo "Creating $MCPPER_JSON with remote_osascript config"
...
' "$MCPPER_JSON" "$MCPPER_JSON.new" > "$MCPPER_JSON.tmp" && mv "$MCPPER_JSON.tmp" "$MCPPER_JSON"
  ...
  echo "Appended remote_osascript to $MCPPER_JSON"
fi
